...
\documentclass[9pt,aspectratio=169]{beamer} %handout para quitar las pausas
...
\setmainfont[Path=/usr/share/fonts/truetype/dejavu/,
BoldItalicFont=DejaVuSans-BoldOblique.ttf,
BoldFont      =DejaVuSans-Bold.ttf,
ItalicFont    =DejaVuSans-Oblique.ttf]{DejavuSans.ttf}
%
...
\author{Jorge de la Vega Góngora}
...
Adicionalmente, hay varios IDEs para trabajar de manera interactiva con \python: {\tt Ipython} + editor de texto, {\tt Jupyter}, {\tt Spyder}, {\tt PyDev}, {\tt Atom}, etc.
...
\begin{frame}[fragile, allowframebreaks]{Ambientes (environments)}
	\begin{itemize}
		...
		\end{itemize} 
	    \item Para crear un ambiente virtual de con nombre tuambiente:
\begin{verbatim}
conda create -n tuambiente python=x.x anaconda
\end{verbatim}   
         El comando anterior instalará la versión de \python\ indicada y todos los paqetes anaconda asociados en el directorio en {\tt tu\_anaconda3\_dir/env/tuambiente}. Por ejemplo en mi caso:
         \begin{center}
...
\end{verbatim}
     	...
     	\end{center} 
        \item Para terminar una sesión en el ambiente actual: {\tt conda deactivate}. Esto regresará al ambiente base, que también se puede desactivar con {\tt conda config --set auto\_activate\_base false}
	\end{itemize}
\end{frame}
...
\begin{frame}{Bibliotecas o paquetes}
	\begin{itemize}
		...
		\begin{itemize}
			...
			\item \href{http://www.scipy.org}{{\tt Scipy}}: colección de subpaquetes y funciones que se usan comunmente en ciencias y en finanzas.
			...
			\item \href{http://scikit-learn.org}{{\tt scikit-learn}}: Paquetes para ML: estimación, clasificación, conglomerados.
...
Los datos del libro de Hilpisch pueden obtenerse de su \href{https://github.com/yhilpisch/py4fi2nd/tree/mastplter/source}{sitio de Github}.	
<<engine= 'python', size = "tiny">>=
...
<<"Perf", engine='python', size = "tiny">>=	
import math
import timeit

print(timeit.timeit(stmt='loops = 3000000; a  = range(1, loops); def f(x): return 3 * math.log(x) + math.cos(x) ** 2; r =[f(x) for x in a]' ))
@
\end{frame}
